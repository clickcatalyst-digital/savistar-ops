  ...
  const rows = await queryAll(`
    SELECT e.id, e.name, e.profession, e.pay_type, e.pay_rate, e.active,
      COALESCE((SELECT SUM(CASE a.status WHEN 'present' THEN 1 WHEN 'half' THEN 0.5 ELSE 0 END)
        FROM attendance a WHERE a.employee_id = e.id AND strftime('%Y-%m', a.date) = ?), 0) AS days_present,
      COALESCE((SELECT SUM(x.amount) FROM employee_expenses x
        WHERE x.employee_id = e.id AND x.kind = 'advance' AND strftime('%Y-%m', x.date) = ?), 0) AS advances,
      (SELECT p.id FROM payroll_payments p WHERE p.employee_id = e.id AND p.period = ?) AS payment_id,
      (SELECT p.net FROM payroll_payments p WHERE p.employee_id = e.id AND p.period = ?) AS paid_net,
      (SELECT p.paid_on FROM payroll_payments p WHERE p.employee_id = e.id AND p.period = ?) AS paid_on
    FROM employees e
    WHERE e.active = 1 OR (SELECT COUNT(*) FROM attendance a2 WHERE a2.employee_id = e.id AND strftime('%Y-%m', a2.date) = ?) > 0
    ORDER BY e.name`,
