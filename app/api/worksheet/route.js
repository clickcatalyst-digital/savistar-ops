...
export async function GET(req) {
  ...
  const [employees, attendance, workLogs] = await Promise.all([
    queryAll('SELECT * FROM employees WHERE active = 1 ORDER BY name'),
    queryAll('SELECT * FROM attendance WHERE date = ?', [date]),
    queryAll(`SELECT w.*, o.item AS order_item FROM work_logs w
              LEFT JOIN orders o ON o.id = w.order_id WHERE w.date = ? ORDER BY w.id`, [date]),
  ]);
