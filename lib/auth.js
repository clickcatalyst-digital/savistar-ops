...
// Cash entries: owners/admins/managers can touch any entry; staff only the ones they created.
export function canAccessCash(user, createdBy) {
  return isApprover(user) || (isStaff(user) && !!user && user.username === createdBy);
}
