...
// Seed the real logins so the app is usable on first run: two owners (admin) + office staff.
// ponytail: default password is <username>123 for each — change them in Settings right after first login.
async function seedIfEmpty(client) {
  ...
  await mk('hari', 'admin', 'Hari');
  await mk('sachi', 'admin', 'Sachi');
